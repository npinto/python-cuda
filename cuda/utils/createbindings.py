...
def main(args=None):
    ...
    config = {'HEADER_FILE':options.HEADER_FILE, 'XML_FILE': options.XML_FILE, 'PYTHON_FILE': options.PYTHON_FILE, 'INCLUDE': options.INCLUDE_DIRS}
    h2xmlcmd = 'python -m ctypeslib.h2xml %(HEADER_FILE)s -I %(INCLUDE)s -o %(XML_FILE)s' % config
    print "h2xmlcmd = %s" % h2xmlcmd
    p = Popen(h2xmlcmd, shell=True)
    sts = waitpid(p.pid, 0)

    xml2pycmd = ('python -m ctypeslib.xml2py %(XML_FILE)s -c -d -v ' % config) + ' '.join(library_options)
